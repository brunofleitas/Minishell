...
void		restore_io_fds(t_ma *ma, int *fix_stdin, int *fix_stdout, int *fix_stderr);
...
void        handle_regular_substring(const char **s, char ***split, int *i, t_ma *ma);
...
char        *process_dollar_expansion(const char **s, char *temp, int word_length, t_ma *ma);
char        *ft_strremove_quotes(const char *str, t_ntc **first_node);

void        append_env_value(const char *temp, int *i, char **result, t_ma *ma);
char		*get_env_var_value(const char *s, int word_length, int len_var, t_ma *ma);
char        *append_remaining_word(const char *s, int word_length, int len_var, t_ma *ma);
...
void 		add_redir(t_astnode **first_redir_node, t_astnode **current, t_astnode *redir_node);
void 		check_redirection_syntax(t_ma *ma);
void		process_redirection(t_astnode *node, t_astnode **last_word, t_astnode *redir_node, t_astnode **current);
void		parse_redirection_list(t_astnode *node, t_astnode **last_word, t_ma *ma);

int			is_word_token(t_token_type type);
void		add_word(t_astnode **first_word_node, t_astnode **current, t_astnode *word_node);
...
void		child_process(t_pip_args *a, t_astnode *simple_cmd, t_astnode *cmd_redir, t_ma *ma);
...
int			create_tmp_file(const char *temp_file_name, const char *delimiter, t_ma *ma);
...
char		*find_command_path(char *cmd, t_env **env, int *p_ue, t_ntc **first_node);
